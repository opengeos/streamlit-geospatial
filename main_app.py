...
def load_and_process_metrics(location_id, start_date, end_date):
    ...
    if not st.session_state.get(
        "bq_client"
    ):  # Check if bq_client exists and is not None
    ...
    metrics_df = fetch_historical_metrics(
        st.session_state.bq_client, location_id, start_date, end_date
    )

    if metrics_df is None or metrics_df.empty:
        st.warning(
            f"Failed to fetch metrics for {location_id} from BigQuery or no data available. Using sample data."
        )
        ...
        if len(sample_values) >= 25:
            sample_values[20:25] *= 3
        if len(sample_values) >= 75:
            sample_values[70:75] *= 0.2
        metrics_df = pd.DataFrame(
            {
                "timestamp": sample_dates,
                "metric_value": sample_values,
                "location_id": location_id,
            }
        )

    if (
        metrics_df is not None and not metrics_df.empty
    ):  # Check added for metrics_df not being None
        ...
        if "metric_value" in metrics_df.columns:
            ...
            metrics_df["is_outlier"] = (metrics_df["metric_value"] > upper_bound) | (
                metrics_df["metric_value"] < lower_bound
            )
            ...
            st.session_state.current_metrics_df = (
                metrics_df  # Store even if no metric_value for inspection
            )
...
def handle_location_selection_change():
    ...
    current_start_date = st.session_state.get(
        "metrics_start_date", datetime.date(2023, 1, 1)
    )
...
if "initialized" not in st.session_state:
    ...
    st.session_state.selected_location_id = (
        st.session_state.locations_data["location_id"].iloc[0]
...
if (
    st.session_state.locations_data is not None
    and not st.session_state.locations_data.empty
):
...
if (
    "locations_data" in st.session_state
    and st.session_state.locations_data is not None
    and not st.session_state.locations_data.empty
):
    df_map = st.session_state.locations_data
    if (
        isinstance(df_map, pd.DataFrame)
        and "latitude" in df_map.columns
        and "longitude" in df_map.columns
    ):
        if not df_map.empty:
        ...
        m.add_points_from_xy(
            df_map, x="longitude", y="latitude", popups=["location_id"]
        )
        m.to_streamlit(height=500)
    else:
        st.warning(
            "Location data is available but missing 'latitude' or 'longitude' columns, or is not a DataFrame."
        )
else:
...
if st.session_state.get("current_schematic_path"):
    st.image(
        st.session_state.current_schematic_path,
        caption=f"Schematic for {st.session_state.get('selected_location_id', 'N/A')}",
    )
elif st.session_state.get(
    "selected_location_id"
):  # If a location is selected but path is None
...
with col1:
    st.date_input(
        "Start Date", key="metrics_start_date", on_change=handle_metrics_date_change
    )
with col2:
    st.date_input(
        "End Date", key="metrics_end_date", on_change=handle_metrics_date_change
    )

if st.session_state.get(
    "selected_location_id"
):  # Only show metrics if a location is selected
    if (
        st.session_state.get("current_metrics_df") is not None
        and not st.session_state.current_metrics_df.empty
    ):
        ...
        if (
            "timestamp" in current_metrics_df.columns
            and "metric_value" in current_metrics_df.columns
        ):
            ...
            if current_outliers_df is not None and not current_outliers_df.empty:
                st.dataframe(
                    current_outliers_df[["timestamp", "metric_value", "is_outlier"]]
                )
            else:
        ...
        "selected_location_id"
    ):  # If selected_id exists but no metrics_df
        st.info(
            f"No metrics data loaded or available for {st.session_state.selected_location_id}. Try adjusting dates or ensure data source is available."
        )
else:
...
if st.button("Get Prediction from Vertex AI"):
    ...
    if endpoint:
        ...
        st.error(
            "Failed to initialize Vertex AI endpoint. Check configuration, authentication, and endpoint ID."
        )
