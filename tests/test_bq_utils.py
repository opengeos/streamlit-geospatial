...
def test_get_bigquery_client_failure():
    with patch(
        "google.cloud.bigquery.Client", side_effect=GoogleAPICallError("Auth error")
    ):
        client = bq_utils.get_bigquery_client(project_id="test-project")
...
def test_fetch_historical_metrics_success(mock_bq_client):
    ...
    mock_query_job.to_dataframe.return_value = pd.DataFrame(
        {
            ...
            ),  # Ensure it's a valid timestamp format
            "metric_value": [100.5],
        }
    )
    mock_bq_client.query.return_value = mock_query_job

    df = bq_utils.fetch_historical_metrics(
        mock_bq_client, "LOC001", date(2023, 1, 1), date(2023, 1, 31)
    )
    ...
    job_config_params = {
        p.name: p.value for p in call_args.kwargs["job_config"].query_parameters
    }
...
def test_fetch_historical_metrics_no_timestamp_column(mock_bq_client):
    ...
    with patch("builtins.print") as mock_print:
        ...
        mock_print.assert_any_call(
            "Warning: 'timestamp' column not found in metrics data."
        )
